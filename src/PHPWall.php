...
class PHPWall
{
    ...
    public function isTrustIp($host)
    {
        foreach ($this->trustHosts as $r) {
            if (is_string($r) && strpos($host, $r) !== false) {
                return true;
            }
    ...
    public function restoreCache()
    {
        ...
        foreach ($data as $r) {
            try {
                $ip = Tools::convertIp2String((string)$r['ip']);
                $this->cache->setIpCache($ip, $this->getBunTimeout($r), (int)$r['trust']);
            } catch (\InvalidArgumentException $e) {
                $this->log(LogLevel::WARNING, "Could not restore cache for invalid IP format: {$e->getMessage()}");
            }
        }
    }
